...
def inventory_count(inventory, *keywords):
    """Count items in inventory by loose keyword matching."""
    count = 0
...
def inventory_counts(inventory):
    return {
        "stack": inventory_count(inventory, "stack"),
        "sword": inventory_count(inventory, "sword"),
        "boost": inventory_count(inventory, "boost", "speed"),
        "star": inventory_count(inventory, "star", "shield"),
    }


def reserve_target(snakes, my_head):
    """Dynamic reserve target: conserve more early, spend more late."""
    enemy_count = sum(1 for s in snakes if s.get("head") != my_head)
    if enemy_count >= 4:
        return 3       # early/crowded: build buffer
    if enemy_count >= 2:
        return 2       # mid game: normal reserve
    return 1           # late game: spend more aggressively


def usage_mode(count, reserve):
    if count <= max(0, reserve - 1):
        return "critical"      # 0-1 below reserve: very conservative
    if count <= reserve:
        return "reserve"       # preserve unless necessary
    if count <= reserve + 2:
        return "normal"        # normal usage allowed
    return "aggressive"        # excess items: spend freely


def can_spend(count, reserve, urgency=0):
    """
    Inventory-aware item spending gate.
    urgency: 0 low, 1 useful, 2 strong, 3 emergency.
    Emergency always overrides conservation.
    """
    if urgency >= 3:
        return count > 0
    if count <= 0:
        return False
    mode = usage_mode(count, reserve)
    if mode == "critical":
        return urgency >= 3
    if mode == "reserve":
        return urgency >= 2
...
def reserve_pickup_bonus(item_count, reserve):
    """Bonus for collecting reserve items when below target."""
    if item_count < reserve:
        return 220 * (reserve - item_count)
    if item_count == reserve:
        return 60
    return max(-40, 30 - 15 * (item_count - reserve))


def item_usage_penalty(item_count, reserve, urgency):
    """Penalty for spending scarce resources without enough urgency."""
    if urgency >= 3:
        return 0
    if item_count <= reserve - 1:
        return 900
    if item_count <= reserve and urgency < 2:
        return 450
    if item_count <= reserve + 2 and urgency == 0:
        return 160
    return 0
...
def choose_next_move(my_head, obstacles, snakes, size, items, direction):
    ...
    reserve = reserve_target(snakes, my_head)
    ...
    # Emergency context used for resource conservation overrides.
    best_clean_space = max((flood_fill(pos, normal_blocked, size, limit=250) for pos in clean_moves.values()), default=0)
    exposure_now = body_exposure_score(my_body, enemy_maps)
    self_trap_emergency = (not clean_moves) or (best_clean_space < 10 and len(my_body) >= 5)
    stack_urgency = 3 if self_trap_emergency else (2 if exposure_now >= 1600 else (1 if exposure_now >= 1200 else 0))
    ...
    if should_activate_stack(my_body, clean_moves, normal_blocked, enemy_maps, size, inventory):
        if _tick - _last_stack > 2 and can_spend(inv_counts["stack"], reserve, stack_urgency):
            activate = "STACK"
            _last_stack = _tick
    ...
    if activate is None and sword_available:
        ...
        sword_urgency = 3 if not clean_moves else 2
        if cut_move is not None and _tick - _last_sword > 1 and can_spend(inv_counts["sword"], reserve, sword_urgency):
            _last_sword = _tick
    ...
    for move, pos in clean_moves.items():
        ...
        # 5. Valuable items.
        if pos in stacks:
            score += 520 + reserve_pickup_bonus(inv_counts["stack"], reserve)
        if pos in swords:
            score += 480 + reserve_pickup_bonus(inv_counts["sword"], reserve)
        if pos in boosts or pos in speeds:
            score += 330 + reserve_pickup_bonus(inv_counts["boost"], reserve)
        ...
        if has_boost(inventory):
            if validates_boost_path(my_head, move, normal_blocked, enemy_maps, bad_set, size, allow_bad=False):
                score += 60
                if usage_mode(inv_counts["boost"], reserve) in ("critical", "reserve"):
                    score -= 35  # do not shape the whole plan around scarce boost unless needed
            else:
                score -= 250
    ...
    if activate is None and has_stack(inventory):
        ...
        final_space = flood_fill(final_pos, normal_blocked, size, limit=200)
        exposure = body_exposure_score(my_body, enemy_maps)
        stack_urgency = 3 if final_space < 8 else (2 if exposure > 1800 or final_space < 12 else (1 if exposure > 1400 else 0))
        if (_tick - _last_stack > 3 and
                can_spend(inv_counts["stack"], reserve, stack_urgency) and
                (final_space < 10 or exposure > 1600)):
            activate = "STACK"
            _last_stack = _tick
    ...
    if activate is None and sword_available:
        ...
        for s in snakes:
            ...
            if final_pos in body[1:] and final_pos not in enemy_maps["danger_1"]:
                idx = body.index(final_pos) if final_pos in body else len(body) - 1
                cut_gain = max(1, len(body) - idx)
                sword_urgency = 3 if self_trap_emergency else (2 if cut_gain >= 3 else 1)
                if _tick - _last_sword > 1 and can_spend(inv_counts["sword"], reserve, sword_urgency):
                    activate = "SWORD"
                    _last_sword = _tick
                break

    if activate is None and has_boost(inventory):
        # Use boost only when both cells are validated and it helps escape/race, while preserving reserves.
        if _tick - _last_boost > 4 and validates_boost_path(my_head, best_move, normal_blocked, enemy_maps, bad_set, size):
            danger_now = cell_risk_score(final_pos, enemy_maps) > 600 or best_clean_space < 14
            item_race = final_pos in boosts or final_pos in speeds or final_pos in swords or final_pos in stacks
            multi_apple_route = nearest_distance(final_pos, apples, size) is not None and flood_fill(final_pos, normal_blocked, size, limit=300) > 55
            boost_urgency = 3 if danger_now else (2 if item_race else (1 if multi_apple_route else 0))
            if can_spend(inv_counts["boost"], reserve, boost_urgency) and (danger_now or item_race or multi_apple_route):
                activate = "BOOST"
                _last_boost = _tick
    ...
    if guarded_pos in hard_blocked and activate != "STACK":
        ...
        elif has_stack(inventory) and can_spend(inv_counts["stack"], reserve, 3):
            activate = "STACK"
